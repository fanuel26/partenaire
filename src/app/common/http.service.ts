...
export class HttpService {
  ...
  postUploadRequestOther(url: string, data: FormData, option?: any): Observable<any> {
    return this.http.post(`${url}`, data, option).pipe(catchError(this.handleError));
  }
  postSemblantRequest(url: string, data: any, option?: any): Observable<any> {
    return this.http.get(`${API_USERS_URL}${url}`, data).pipe(catchError(this.handleError));
  }
  ...
  handleError(error: HttpErrorResponse) {
    if (error.error instanceof ErrorEvent) {
      ...
      //if erreur de ser par ex ou 403 et autres
